...
任务归档后可以在“已归档任务”页恢复，保留原任务编号。系统页提供工作台备份导出、恢复预览与确认，以及 Vault 移动或改名后的显式工作区重新绑定。这些权限独立于 Vault 只读权限：项目只读时仍可导出和查看候选工作区，但不能恢复或重新绑定；托管版本不提供这些本地操作。Obsidian 总览不显示抖音指标及其来源提示。
