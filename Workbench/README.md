...
Workbench 提供可复用的读取、索引和可视化能力，不包含任何作者的个人知识、账号数据、登录态或历史运行记录。仓库级 `个人知识库/` 是完全虚构的演示 Vault，用于展示素材阅读、Wiki、知识星图、灵感和内容页面。
...
新项目数据保存在系统用户应用数据目录 `PersonalAIWorkbench/workspaces/<workspace-id>/projects/projects.json`，不写入 Vault，也不进入 Git 仓库。注册表把 Vault 指纹关联到稳定的工作区 ID；旧版 `PersonalAIWorkbench/<vault-id>/projects/projects.json` 会被原地接管，不移动文件。写入采用进程间票据锁、修订号冲突保护和原子替换，恢复事务在清理完成前持续持锁。项目 API 只接受同源、回环地址的本地请求。Vault 只读模式不会阻止外部项目数据编辑；如需同时锁定项目数据，可设置 `WORKBENCH_PROJECTS_READ_ONLY=true`。静态托管构建不显示项目导航和项目路由。

任务归档后可以在“已归档任务”页恢复，保留原任务编号。系统页提供工作台备份导出、恢复预览与确认，以及 Vault 移动或改名后的显式工作区重新绑定。这些权限独立于 Vault 只读权限：项目只读时仍可导出和查看候选工作区，但不能恢复或重新绑定；托管版本不提供这些本地操作。

备份只包含受支持的本地状态提供方数据，不包含 Vault 正文、凭据或缓存。导入会校验版本、校验和、记录唯一性和所属关系；未知版本不能覆盖当前文件。有效备份可以修复损坏的项目 JSON，当前文件的原始字节会暂存为回滚材料；后续提供方提交失败时恢复原始字节。恢复有容量限制，不会跟随符号链接或联接点。当前实现处理正常返回的提交失败，但没有跨提供方进程崩溃事务日志；不要把断电或强制终止期间的多提供方恢复视作崩溃原子事务。

移动 Vault 后即使先访问了空项目页，也可以在系统页选择旧工作区并确认重新绑定。仅允许替换没有状态文件的临时绑定；确认前若该工作区已有新数据，会拒绝重新绑定而保留数据。其他已启动的本地服务若仍缓存旧绑定，其写入会被拒绝，需要重启服务再继续。
