...
import { isObsidianPath, obsidianMaterialPath, extractObsidianText } from "./obsidian-vault.mjs";
...
function classifyDocument(relativePath, profile = "default") {
  const parts = relativePath.split("/");
  const top = parts[0] || "";
  const section = parts.length > 2 ? parts[1] : null;

  if (profile === "obsidian" && top !== "wiki") {
  ...
  if (top === "wiki") {
    return {
      layer: "wiki",
      section,
      kind: (profile === "obsidian" || FORMAL_WIKI_SECTIONS.has(section))
        ? "knowledge"
        : section === "templates"
          ? "template"
          : section === "usage"
...
async function collectFiles(vaultRoot, errors, profile) {
  const files = [];
  ...
  async function walk(absoluteDirectory) {
    ...
    for (const entry of entries) {
      if (entry.name.startsWith(".") && !(profile === "obsidian" && absoluteDirectory === vaultRoot && entry.name === ".raw")) continue;
      const absolutePath = path.join(absoluteDirectory, entry.name);
      const relativePath = safeRelative(vaultRoot, absolutePath);
      if (!relativePath) continue;
      if (profile === "obsidian" && !isObsidianPath(relativePath)) continue;
...
async function buildDocument(file, vaultRoot, errors, profile) {
  let stats;
  try {
  ...
  const classification = classifyDocument(file.relativePath, profile);
  const fallbackTitle =
  ...
  if (profile === "obsidian") {
    const extracted = await extractObsidianText(vaultRoot, file.relativePath, stats.size);
    rawContent = extracted.content;
    contentStatus = extracted.contentStatus;
    if (rawContent != null) {
      if (extension === "md") parsed = parseMarkdown(rawContent);
      else parsed.body = rawContent;
    }
    if (contentStatus === "extraction_failed") errors.push({ path: file.relativePath, code: "TEXT_EXTRACTION_FAILED" });
  ...
  const document = {
    ...
    materialPath: profile === "obsidian" && classification.layer === "raw" ? obsidianMaterialPath(file.relativePath) : undefined,
    contentStatus,
    ...
    modifiedAt,
    isArchived,
    previewKind: profile === "obsidian" && rawContent != null && extension !== "md" ? "text" : previewKind(extension),
    frontmatter,
...
function publicDocument(document) {
  return {
    ...
    materialPath: document.materialPath,
 ...
 */
export async function buildVaultIndex(vaultRoot, { profile = process.env.PERSONAL_DASHBOARD_PROFILE || "default" } = {}) {
  ...
  const errors = [];
  const files = await collectFiles(resolvedRoot, errors, profile);
  const documents = [];

  for (const file of files) {
    try {
      const document = await buildDocument(file, resolvedRoot, errors, profile);
