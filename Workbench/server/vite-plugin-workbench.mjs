...
async function serveReaderImage(res, index, vaultRoot, sourceId, source) {
  ...
  if (index.profile === "obsidian") {
    try {
      const buffer = await readIndexedFile(vaultRoot, document.path, maximumVaultImageBytes);
      res.writeHead(200, { "Content-Type": contentType, "X-Content-Type-Options": "nosniff", "Content-Security-Policy": "default-src 'none'; sandbox", "Cache-Control": "no-store" });
      res.end(buffer);
      return;
    } catch {
      return json(res, 404, { error: { code: "READER_IMAGE_NOT_FOUND", message: "文章图片不可访问。" } });
    }
  }
...
function documentPayload(index, id) {
  ...
  const body = document.extension === "md" ? readerBodyFromContent(document.content) : document.content;
  return {
    ...document,
...
export function workbenchApiPlugin({
  ...
  const materialReadingState = readOnly ? { list: async () => ({ items: [], updatedAt: null }) } : createMaterialReadingStateRepository({ vaultRoot });
  const wikiIngest = createWikiIngestRunner({ vaultRoot });
  const readerExplanations = readOnly ? {} : (readerExplanationService ??
    createReaderExplanationsService({ vaultRoot }));
  const vaultSync = createVaultSyncService({ vaultRoot, buildIndex: async (root) => {
    const index = await buildVaultIndex(root, { profile });
    index.readOnly = readOnly;
    for (const document of index.documents) document.readOnly = readOnly;
    return index;
  } });
  ...
  return {
    ...
    async configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (req.method === "GET" && url.pathname.startsWith("/api/files/")) {
            ...
            const document = getDocument(await currentIndex(), id);
            if (!document) return json(res, 404, { error: { message: "文档不存在。" } });
            try {
              const data = await readIndexedFile(vaultRoot, document.path, 64 * 1024 * 1024);
              res.writeHead(200, { "Content-Type": "application/octet-stream", "Content-Disposition": `attachment; filename*=UTF-8''${encodeURIComponent(document.fileName).replace(/'/g, "%27")}`, "X-Content-Type-Options": "nosniff", "Cache-Control": "no-store" });
              res.end(data);
              return;
            } catch {
              return json(res, 404, { error: { message: "文件已移动、不可访问或超过 64 MiB 下载上限。" } });
            }
          }
          ...
          if (req.method === "GET" && url.pathname.startsWith("/api/documents/")) {
            ...
            if (!document) return json(res, 404, { error: { message: "文档不存在。" } });
            return json(res, 200, { ...document, readOnly });
          }
          ...
          if (req.method === "GET" && url.pathname === "/api/runtime") {
            const [current, codex] = await Promise.all([currentIndex(), readOnly ? { available: false, source: null } : detectCodexCli()]);
            return json(res, 200, {
              readOnly,
              profile,
              vault: {
      ...
      // Finish the cold scan before Vite announces readiness. Large local
      // libraries otherwise outlive the browser's initial API timeout.
      if (profile === "obsidian") await currentIndex();
    },
  };
}
