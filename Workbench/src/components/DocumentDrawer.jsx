...
export function DocumentDrawer({ documentId, onClose, onNavigateDocument, readingContext, onAskDocument, assistantOpen = false }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      if (assistantOpen && (event.target?.closest?.(".knowledge-panel") || event.key === "Tab")) return;
      if (event.key === "Escape") {
  ...
  }, [activeImage, closeImagePreview, documentId, assistantOpen]);
  ...
  const captureReaderSelection = useCallback(() => {
    if (currentDocument?.readOnly) return;
    if (selectionFrameRef.current) window.cancelAnimationFrame(selectionFrameRef.current);
  ...
  const markdownComponents = useMemo(() => {
    ...
    return {
      ...
      img({ node, src, alt, ...props }) {
        if (currentDocument?.readOnly && /^(?:https?:)?\/\//i.test(src || "")) return <span>外部图片未加载：{alt || "图片"}</span>;
  ...
  return (
    <motion.section
      ...
      role="dialog"
      aria-modal={assistantOpen ? "false" : "true"}
    ...
    >
      ...
      <header className="reader__header" aria-hidden={activeImage ? "true" : undefined}>
        ...
        <div className="reader__actions">
          {currentDocument && onAskDocument && <button type="button" className="reader__queue" onClick={() => onAskDocument(currentDocument)}>加入问答</button>}
          {currentDocument ? (
            <>
              {currentDocument.layer === "raw" && !currentDocument.readOnly ? (
                <button
              ...
              {currentDocument.readOnly ? <a className="icon-button" href={`/api/files/${encodeURIComponent(currentDocument.id)}`} download aria-label="下载原文件" title="下载原文件（不会执行脚本）"><IconExternalLink aria-hidden="true" /></a> : <>
              <button
      ...
      >
        {state.loading ? (
          ...
          <div
            className={`reader__layout${railCollapsed ? " reader__layout--rail-collapsed" : ""}${workspaceCollapsed ? " reader__layout--workspace-collapsed" : ""}${currentDocument.readOnly ? " reader__layout--readonly" : ""}`}
          >
            ...
            <main className="reader__document">
              ...
              {currentDocument.readOnly ? <p className="provenance">本地只读 · 原文件保持不变 · 脚本不会执行</p> : null}
              {currentDocument.contentStatus && currentDocument.contentStatus !== "ready" ? <p className="provenance">{{ too_large: "文件超过正文读取上限（文本 8 MiB，Office/PDF 64 MiB），当前仅索引文件名。", metadata_only: "此格式仅索引文件名，可下载原文件查看。", no_text: "未提取到可搜索文字，可能是扫描文档或空文件；未执行 OCR。", extraction_failed: "正文提取失败，可能存在格式、加密或损坏问题；原文件仍可下载。", truncated: "提取文本过长，正文和搜索仅包含前 8 Mi 字符。" }[currentDocument.contentStatus]}</p> : null}
              ...
              >
                {currentDocument.previewKind === "text" ? <pre className="reader-code-block"><code>{documentBody}</code></pre> : <ReactMarkdown
                  remarkPlugins={[
                    remarkGfm,
                    remarkObsidianCjkStrong,
            ...
            {!currentDocument.readOnly ? <ReaderWorkspace
              ref={workspaceRef}
