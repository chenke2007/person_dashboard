...
export function MaterialsPage({ onOpenDocument }) {
  ...
  const title = isHome
    ? (import.meta.env.VITE_WORKBENCH_PROFILE === "obsidian" ? "DBA 资料与脚本" : "素材层")
    : isQueue
      ? "我的待看"
      : data?.folder?.displayName || folderPath.split("/").pop();
  const description = isHome
    ? (import.meta.env.VITE_WORKBENCH_READ_ONLY === "true" ? "直接读取本地知识库，保留原始目录；支持资料与脚本全文搜索，只读浏览，不执行脚本。" : "原始证据按真实文件夹归位；待看只是阅读安排，不改变素材，也不代表已经入库。")
    : isQueue
  ...
  return (
    <div className="page page--materials">
      ...
      {isLoading ? (
        ...
        >
          {import.meta.env.VITE_WORKBENCH_READ_ONLY !== "true" ? <section className="materials-section materials-section--queue">
