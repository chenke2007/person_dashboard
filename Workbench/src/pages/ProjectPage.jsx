...
export function ProjectView({ snapshot, view, filters, onChangeView, onChangeFilters, onCreateTask, onOpenTask, onMoveTask }) {
  ...
  const changeMany = (field) => (event) => onChangeFilters({ ...filters, [field]: [...event.target.selectedOptions].map((option) => option.value) });
  return <section className="project-page page-shell"><Link className="project-back" to="/projects"><IconArrowLeft />所有项目</Link><header className="project-heading"><div><span>{snapshot.project.key}</span><h1>{snapshot.project.name}</h1><p>{snapshot.project.description || "尚未填写项目说明"}</p></div><button className="project-button project-button--primary" onClick={onCreateTask} type="button"><IconPlus />新建任务</button></header>
    ...
    <div className="project-filters"><label>状态<select onChange={change("columnIds")} value={filters.columnIds?.[0] || ""}><option value="">全部</option><option value="backlog">Backlog</option>{snapshot.columns.map((column) => <option key={column.id} value={column.id}>{column.name}</option>)}</select></label><label>优先级<select onChange={change("priorities")} value={filters.priorities?.[0] || ""}><option value="">全部</option><option value="urgent">紧急</option><option value="high">高</option><option value="medium">中</option><option value="low">低</option></select></label><label>标签（可多选）<select aria-label="标签筛选" multiple onChange={changeMany("labelIds")} value={filters.labelIds || []}>{snapshot.labels.map((label) => <option key={label.id} value={label.id}>{label.name}</option>)}</select></label><label>截止从<input onChange={(event) => onChangeFilters({ ...filters, dueAfter: event.target.value })} type="date" value={filters.dueAfter || ""} /></label><label>截止至<input onChange={(event) => onChangeFilters({ ...filters, dueBefore: event.target.value })} type="date" value={filters.dueBefore || ""} /></label></div>
