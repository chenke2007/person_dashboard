...
export function ProjectView({ snapshot, view, filters, onChangeView, onChangeFilters, onCreateTask, onOpenTask, onMoveTask, onOpenSettings }) {
  ...
  return <section className="project-page page-shell"><Link className="project-back" to="/projects"><IconArrowLeft />所有项目</Link><header className="project-heading"><div><span>{snapshot.project.key}</span><h1>{snapshot.project.name}</h1><p>{snapshot.project.description || "尚未填写项目说明"}</p></div><div className="project-heading__actions"><button className="project-button" onClick={onOpenSettings} type="button"><IconSettings />项目设置</button><button className="project-button project-button--primary" onClick={onCreateTask} type="button"><IconPlus />新建任务</button></div></header>
    ...
    <div className="project-filters"><label>状态<select onChange={change("columnIds")} value={filters.columnIds?.[0] || ""}><option value="">全部</option><option value="backlog">Backlog</option>{snapshot.columns.map((column) => <option key={column.id} value={column.id}>{column.name}</option>)}</select></label><label>优先级<select onChange={change("priorities")} value={filters.priorities?.[0] || ""}><option value="">全部</option><option value="urgent">紧急</option><option value="high">高</option><option value="medium">中</option><option value="low">低</option></select></label><div className="project-label-filter"><span>标签</span><details><summary>标签：{filters.labelIds?.length ? `已选 ${filters.labelIds.length} 个` : "全部"}</summary><div>{snapshot.labels.length ? snapshot.labels.map((label) => <label key={label.id}><input checked={(filters.labelIds || []).includes(label.id)} onChange={() => toggleLabelFilter(label.id)} type="checkbox" /><i style={{ background: label.color }} />{label.name}</label>) : <p>暂无标签</p>}</div></details></div><label>截止从<input onChange={(event) => onChangeFilters({ ...filters, dueAfter: event.target.value })} type="date" value={filters.dueAfter || ""} /></label><label>截止至<input onChange={(event) => onChangeFilters({ ...filters, dueBefore: event.target.value })} type="date" value={filters.dueBefore || ""} /></label></div>
