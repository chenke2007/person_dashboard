...
import { addTaskLink, archiveTask, createTask, loadProject, moveTask, removeTaskLink, setTaskLabels, updateTask } from "../lib/project-api.js";
...
function List({ snapshot, tasks, onOpenTask }) {
  return <div className="project-list" role="table"><div className="project-list__head" role="row"><span>任务</span><span>状态</span><span>优先级</span><span>截止日期</span></div>{tasks.map((task) => <button className="project-list__row" key={task.id} onClick={() => onOpenTask(task)} role="row" type="button"><span>{snapshot.project.key}-{task.number} · {task.title}</span><span>{snapshot.columns.find((column) => column.id === task.columnId)?.name || "Backlog"}</span><span>{task.priority}</span><span>{task.dueDate || "—"}</span></button>)}</div>;
}

export function ProjectView({ snapshot, view, filters, onChangeView, onChangeFilters, onCreateTask, onOpenTask, onMoveTask }) {
  ...
  return <section className="project-page page-shell"><Link className="project-back" to="/projects"><IconArrowLeft />所有项目</Link><header className="project-heading"><div><span>{snapshot.project.key}</span><h1>{snapshot.project.name}</h1><p>{snapshot.project.description || "尚未填写项目说明"}</p></div><button className="project-button project-button--primary" onClick={onCreateTask} type="button"><IconPlus />新建任务</button></header>
    ...
    <div className="project-filters"><label>状态<select onChange={change("columnIds")} value={filters.columnIds?.[0] || ""}><option value="">全部</option><option value="backlog">Backlog</option>{snapshot.columns.map((column) => <option key={column.id} value={column.id}>{column.name}</option>)}</select></label><label>优先级<select onChange={change("priorities")} value={filters.priorities?.[0] || ""}><option value="">全部</option><option value="urgent">紧急</option><option value="high">高</option><option value="medium">中</option><option value="low">低</option></select></label><label>标签<select onChange={change("labelIds")} value={filters.labelIds?.[0] || ""}><option value="">全部</option>{snapshot.labels.map((label) => <option key={label.id} value={label.id}>{label.name}</option>)}</select></label><label>截止于<input onChange={(event) => onChangeFilters({ ...filters, dueBefore: event.target.value })} type="date" value={filters.dueBefore || ""} /></label></div>
    {view === "board" ? <Board onMoveTask={onMoveTask} onOpenTask={onOpenTask} snapshot={snapshot} tasks={visible} /> : <List onOpenTask={onOpenTask} snapshot={snapshot} tasks={projected} />}
  </section>;
}

export function ProjectPage({ onOpenDocument }) {
  const { projectId } = useParams();
  const [snapshot, setSnapshot] = useState(null); const [error, setError] = useState(null); const [selectedTask, setSelectedTask] = useState(null);
  ...
  const move = async (task, columnId, index) => { const previous = snapshot; setError(null); try { setSnapshot(await moveTask(task.id, { columnId, index, revision: snapshot.revision })); } catch (moveError) { setSnapshot(previous); setError(moveError); } };
  ...
  return <><ProjectView filters={filters} onChangeFilters={setFilters} onChangeView={changeView} onCreateTask={() => setSelectedTask({ id: null, title: "", description: "", priority: "medium", startDate: "", dueDate: "" })} onMoveTask={move} onOpenTask={setSelectedTask} snapshot={snapshot} view={view} />
    <TaskDrawer activities={snapshot.activities} labels={snapshot.labels} links={snapshot.taskLinks} onAddLink={(documentId) => mutateAndRefresh(() => addTaskLink(selectedTask.id, documentId))} onArchive={() => mutateAndRefresh(() => archiveTask(selectedTask.id)).then(() => setSelectedTask(null))} onClose={() => setSelectedTask(null)} onOpenDocument={onOpenDocument} onRemoveLink={(linkId) => mutateAndRefresh(() => removeTaskLink(linkId))} onSave={save} onSetLabels={(taskId, labelIds) => mutateAndRefresh(() => setTaskLabels(taskId, labelIds))} projectKey={snapshot.project.key} task={selectedTask} taskLabels={snapshot.taskLabels} />
